...
- Clients whose funding source sits in the same jurisdiction as their stated residence never expose this, since the single stated-residency field the schema tracks already answers the only question the screening agent asks; the gap opens specifically when a second jurisdiction, like Country B in the funding source, has rules of its own that nothing in the profile prompts anyone to check

**Root Cause**
The client-profile schema carries exactly one residency value because rule selection has historically been a one-jurisdiction lookup: read the field, apply that jurisdiction's rules. A funding source domiciled elsewhere doesn't fit that lookup's shape, so when the account-opening agent notices Country B's bank account doesn't match the Country A residency on file, the observation has no field to land in and stays in intake notes the screening agent's rule-selection step was never built to read. The screening step then does exactly what its schema asks of it -- looks up Country A, applies Country A's rules -- with no signal that a second jurisdiction's documentation requirement was ever in play.
