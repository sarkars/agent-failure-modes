...
- The mismatch concentrates on orders near a limit's current threshold, since those are precisely the cases where a scenario-conditional finding diverges most from a simple current-state pass/fail check

**Root Cause**
The execution agent's routing logic consumes only the structured pre-trade check result's fixed schema, and that schema was built to capture a pass/fail outcome against static current-state limit calculations, not a scenario-conditional finding about how the order interacts with existing positions under a plausible adverse move. Because a marginal-breach risk is expressed through the risk agent's free-text scenario reasoning rather than a structured conditional-risk field, it has no corresponding place in the handoff schema and is therefore invisible to the execution agent, even though the same model, given the risk agent's analysis, would readily flag the order as conditionally risky.
