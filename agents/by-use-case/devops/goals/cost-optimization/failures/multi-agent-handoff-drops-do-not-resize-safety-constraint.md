...
- The dropped exclusion concentrates on instances with exclusions noted as exceptions to an otherwise straightforward rightsizing rule, where the exclusion is most likely to exist only as a one-off comment rather than a structured attribute
...
**Root Cause**
The cost-analysis agent and execution agent are separately invoked components that communicate through a structured candidate list, not through each other's free-text reasoning. When the analysis agent's output contract allows it to note an exclusion only in commentary without requiring a corresponding structured field on the candidate entry, that exclusion is visible to a human reading the analysis report but invisible to the execution agent, which acts on the structured list it is given rather than re-reading the analysis agent's full commentary for every candidate before resizing it.
